...
@section('content')
    ...
    <div class="container">
        ...
        <form action="{{ route('admin.posts.update', $post)}}" id="post" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
        ...
        <form action="{{ route('admin.posts.seo.update', $post)}}" id="seo" method="post">
            @csrf
            @method('PUT')
            ...
            <div class="card mb-3">
                ...
                <div class="card-body pb2 ">
                    <div class="form-group">
                        <label for="title" class="col-form-label">{{ __('messages.title') }}</label>
                        <input id="title" type="text"
                               class="form-control{{ $errors->has('seo_title') ? ' is-invalid' : '' }}" name="seo_title"
                               value="{{ old('seo_title', $post->seo->title) }}" required>
                        @if ($errors->has('title'))
                            <span class="invalid-feedback">
                                    <strong>{{ $errors->first('title') }}</strong>
                                </span>
                    ...
                    <div class="form-group">
                        <label for="seo_description" class="col-form-label">{{ __('messages.description') }}</label>
                        <textarea name="seo_description" id="seo_description"
                                  class="form-control{{ $errors->has('seo_description') ? ' is-invalid' : '' }}"
                                  rows="10">{{ old('seo_description', $post->seo->description) }}</textarea>
                    ...
                    <div class="form-group">
                        <label for="seo_keywords" class="col-form-label">{{ __('messages.keywords') }}</label>
                        <textarea name="seo_keywords" id="seo_keywords"
                                  class="form-control{{ $errors->has('seo_keywords') ? ' is-invalid' : '' }}"
                                  rows="10">{{ old('seo_keywords', $post->seo->keywords) }}</textarea>
